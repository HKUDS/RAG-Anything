...
Enable users to download original uploaded files from the knowledge base via the frontend UI. Supports three entry points: table action column button, clickable filename, and detail panel button. Backend supports dual-mode authentication (Authorization header + `?token` query parameter) to enable native browser downloads via `<a>` tags.
