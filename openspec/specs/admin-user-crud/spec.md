...
管理员对用户的完整 CRUD 操作能力，含分页、搜索、筛选、排序。

## ADDED Requirements

### Requirement: Admin can create new users
系统 SHALL 提供 `POST /api/admin/users` 端点，允许管理员创建新用户。创建时 MUST 设置初始密码并标记 `must_change_password=True`。

#### Scenario: Admin creates user successfully
- **WHEN** 管理员以有效 Token 调用 `POST /api/admin/users` 并提供 `username`, `email`, `password`, `role_id`
- **THEN** 系统创建用户并返回 201，响应包含 `id`, `username`, `email`, `role`, `must_change_password: true`
...
- **WHEN** 管理员创建用户时使用已存在的 `username` 或 `email`
- **THEN** 系统返回 409 Conflict，错误消息指明冲突字段
...
系统 SHALL 提供分页用户列表端点 `GET /api/admin/users`，支持 `page`、`page_size`、`search`、`role`、`status` 查询参数。
...
#### Scenario: Search users by username or email
- **WHEN** 管理员调用 `GET /api/admin/users?search=john`
- **THEN** 系统返回用户名或邮箱包含 "john" 的用户列表（大小写不敏感）
