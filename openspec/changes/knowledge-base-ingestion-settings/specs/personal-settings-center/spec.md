...
The AI section SHALL show current text/VLM profiles, actual model identifier, source, status, available candidates, and collapsed technical detail. The retrieval section SHALL offer balanced, precise, broad, and custom presets; custom SHALL reveal RRF, channel Top K, graph depth, channels, BM25 tokenizer/k1/b, and constraint state. Upload default controls SHALL state that changes affect only subsequently started tasks and SHALL be used only as fallback when a KB has an explicit ingestion default.
