...
def calculate_text_quality_metrics(text: str) -> Dict[str, float]:
    ...
    sentence_length_score = 1.0 - abs(avg_sentence_length - ideal_sentence_length) / ideal_sentence_length
...
def evaluate_question_answer_pair(question: str, question_type: str, evidence_info: List[Dict]) -> Dict[str, Any]:
    """评估单个问题-答案对"""
    ...
    evidence_metrics = {
        ...
        'evidence_coverage': min(1.0, len(simulated_evidence) / max(len(evidence_info), 1)),
...
def run_testb_simple_evaluation():
    ...
    for paper_id, paper_data in dataset.items():
        ...
        for i, (question, qtype, evidence) in enumerate(zip(questions, question_types, evidential_info)):
            total_questions += 1
            stats[qtype] += 1
