...
def _ensure_vision_model_or_exit(image_root_exists: bool) -> Optional[str]:
    ...
    if vision_model not in names:
        ...
        print(
            "或选择已安装模型，例如 qwen2.5-vl:7b-instruct / llama3.2-vision:11b / llava:7b"
        )
        raise SystemExit(1)
...
class ComprehensiveSPIQATester:
    def __init__(self, image_root: str = ""):
        ...
        self.vision_model_name = (
            os.getenv("AZURE_OPENAI_VISION_DEPLOYMENT", "").strip()
            or os.getenv("VISION_MODEL", "").strip()
            or None
        )

    def load_images_for_paper(
        self, paper_id: str, figures: Dict[str, Any]
    ) -> Dict[str, str]:
        """加载论文的所有图片"""
        images = {}
                ...
                if os.path.exists(image_path):
                    try:
                        with open(image_path, "rb") as f:
                            image_data = f.read()
                            images[fig_id] = base64.b64encode(image_data).decode(
                                "utf-8"
                            )
    ...
    async def process_question(
        ...
        try:
            ...
            use_image = bool(
                reference and reference in images and self.vision_model_name
            )
            ...
            if use_image:
                ...
                mime = (
                    "image/png" if reference.lower().endswith(".png") else "image/jpeg"
                )
                extraction_system = (
            ...
            result_obj = {
                ...
                "parsed": {"pred_letter": pred_letter, "gt_letter": gt_letter},
            }
            if use_image:
                # 附加结构化抽取原文，便于调试与误差分析
                result_obj["vision_extraction"] = (
                    extraction if "extraction" in locals() else ""
                )
            return result_obj
    ...
    async def process_paper(
        self, paper_id: str, paper_content: Dict[str, Any]
    ) -> List[Dict[str, Any]]:
        """处理单篇论文"""
...
async def main():
    ...
    avg_phrase_overlap = (
        sum(phrase_overlaps) / len(phrase_overlaps) if phrase_overlaps else 0
    )
