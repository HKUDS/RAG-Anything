...
12. **原文引用与来源标注规则**：在 FINISH 回答中，每个事实性陈述必须用 `[来源 N]` 标记标注来源。引用 Observation 的具体内容时，用引号直接嵌入原文（至少20字逐字复制），格式：`"原文摘录..."`[来源 1]。不得概括或改写原文。
13. **引用来源块**：FINISH 回答末尾必须附加 `【引用来源】` 部分，格式：`[来源 N] 源文档：文档名 | 原文："被引用的原文..."`。N 与正文 `[来源 N]` 编号一一对应。如果 Observation 中有文档名（如 `(文档：xxx.pdf)`），必须使用该文档名；如果没有文档名，只引原文即可，**绝对不要编造来源名称**。
14. **关联实体引用**：如果 Observation 中包含实体关系信息（如 `→[关系]→`），在回答中引用相关实体关系，格式：`实体A →[关系]→ 实体B`。在 `【引用来源】` 之后附加 `【关联实体】` 块，列出与回答直接相关的实体关系。
15. **输出结构**：FINISH 回答必须按以下顺序组织：回答正文 → 【引用来源】 → 【关联实体】（如有）。不得省略任何必须的部分。
"""
...
最终回答: <综合各步骤后的完整答案，必须包含以下三个部分>
  回答正文：<清晰回答用户问题，每个事实性陈述后标注 [来源 N]>
  【引用来源】：<列出所有引用的来源，每行格式：[来源 N] 源文档：文档名 | 原文："...">
  【关联实体】：（如有实体关系，列出与回答相关的实体关系）

## 规则
1. 每一步分析必须引用检索内容中的具体事实和数据，不得使用你自己的知识
2. **最终回答中的每条事实都必须标注 [来源 N] 标记，并追溯到检索内容的原文。** 不得添加检索内容中没有的信息，不得使用你自己的知识补充或编造
3. 如果检索内容不足以回答问题，在最终回答中明确说明缺少什么信息
4. 最终回答必须基于前面的推理步骤和检索内容
5. 用中文思考和回答
6. **原文引用**：引用检索内容时用引号直接嵌入原文（至少20字逐字复制），格式：`"原文..."`[来源 1]。若有文档名必须使用；若无文档名，只引原文，不要编造来源。
7. **关联实体**：若检索内容包含实体关系路径（如 `→[关系]→`），在回答中引用并在末尾附加 `【关联实体】` 块。
"""
        if context:
            user_prompt = (
                f"## 检索内容\n{context}\n\n"
