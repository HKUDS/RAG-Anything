...
12. **原文引用规则**：在 FINISH 的最终回答中，引用 Observation 的具体内容时，在句末标注：\"原文摘录...\"（来源：文档名）。原文摘录必须从 Observation 中逐字复制（至少20字）。示例：\"面向管理员，提供系统级别数据管理和权限管理\"（来源：毕业论文.pdf）
"""
...
5. 用中文思考和回答
6. **原文引用**：最终回答中引用检索内容时，在句末标注：\"原文摘录...\"（来源：文档名）。原文摘录必须逐字复制（至少20字）。示例：\"面向管理员，提供系统级别数据管理和权限管理\"（来源：毕业论文.pdf）
"""
        if context:
            user_prompt = (
                f"## 检索内容\n{context}\n\n"
