...
12. **原文引用规则**：在 FINISH 的最终回答中，引用 Observation 中的具体事实或数据时，必须用引号直接嵌入原文摘录。示例："系统'面向管理员，提供系统级别数据管理和权限管理'"。原文摘录必须从 Observation 中逐字复制（至少20字），不可概括或改写。不要使用 `[来源 N]` 标记或【引用来源】列表。
"""
...
5. 用中文思考和回答
6. **原文引用**：最终回答中引用检索内容时，用引号直接嵌入原文摘录（至少20字逐字复制）。示例："该系统'面向管理员，提供系统级别数据管理和权限管理'"。不要使用 `[来源 N]` 标记或独立的引用列表。
"""
        if context:
            user_prompt = (
                f"## 检索内容\n{context}\n\n"
