# -*- coding: utf-8 -*-
"""
RAG-Anything Text Chunking Strategies.

Layer: Core
Primary Responsibility: Multiple text chunking strategies — recursive, sentence,
    structure, semantic, agentic. Each strategy conforms to LightRAG chunking_func spec.
Key Dependencies: lightrag.utils (tokenizer, compute_mdhash_id), jieba

Chunking strategies provided:
    recursive_chunking()  — recursive character split with overlap
    sentence_chunking()   — sentence boundary-aware split via jieba
    structure_chunking()  — structure-aware (headings/paragraphs)
    make_semantic_chunking(embed_func) — factory for embedding-based semantic chunking
    make_agentic_chunking(llm_func, model) — factory for LLM-driven agentic chunking

The strategies follow LightRAG chunking_func signature:
    (tokenizer, content, split_by_character, split_by_character_only,
     chunk_overlap_token_size, chunk_token_size) -> list[dict]

where dict format: {"tokens": int, "content": str, "chunk_order_index": int}

Strategies:
    - fixed_size:    Fixed-size cutting (default, zero extra cost)
    - recursive:     Recursive character split (paragraph->sentence->char, zero cost)
    - sentence:      Sentence-level semantic split (zero extra cost)
    - structure:     Document structure-aware (by heading/section, zero cost)
    - semantic:      Semantic similarity chunking (requires Embedding API, medium cost)
    - agentic:       LLM intelligent chunking (requires LLM API, higher cost)
"""
...
# Markdown heading pattern for structure chunking
# Chinese/English sentence split pattern
