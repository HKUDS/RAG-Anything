...
async def get_my_settings(user: dict = Depends(get_current_user)):
    try:
        return await user_settings.get_user_settings(int(user["id"]))
...
async def get_my_settings_options(_user: dict = Depends(get_current_user)):
    try:
        platform = await user_settings.get_platform_settings()
        options = user_settings.settings_options(platform["settings"])
