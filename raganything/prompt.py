...
INLINE_QUOTE_INSTRUCTION = (
    ...
    "3. 如果检索内容中有文档名（如 `(文档：xxx.pdf)`），在句末加`（来源：文档名）`。如果没有文档名，只引原文即可，**不要**自己编造来源名称。\n"
...
ANSWER_FORMAT_INSTRUCTION = (
    ...
    "3. 每条引用必须标注所属文档名（使用`## 可用文档`中的名称）；若上下文无文档名则标\"未知文档\"，不可编造。\n"
    "4. 回答末尾附加 `【引用来源】` 块，每行格式：`[来源 N] 源文档：文档名 | 原文：\"...\"`。每个条目必须包含文档名。\n"
