...
INLINE_QUOTE_INSTRUCTION = (
    "## 引用原文要求（必须严格遵守）\n"
    "1. 引用检索内容时，在句末用以下格式标注："
    "\"原文摘录...\"（来源：文档名）\n"
    "2. 文档名取检索内容中 `[来源 N] (文档：xxx)` 标签里的文档名称，"
    "不要使用 `[来源实体]` 中的实体名作为来源。\n"
    "3. 每个要点只引用 1-2 处关键原文即可，不要罗列所有来源，保持回答简洁流畅。\n"
    "4. 原文摘录必须从检索内容中逐字复制（至少20字），不可概括或改写。\n"
    "5. 如果检索内容中没有相关信息，说明\"未找到\"即可，不要编造。"
)
