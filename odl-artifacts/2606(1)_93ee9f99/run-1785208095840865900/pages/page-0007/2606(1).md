...
the current harness. For a proposal ∆j, let h(tj) = ∆j(ht) denote the resulting candidate harness. We evaluate both the current harness ht and the candidate harness h(tj) on the held-in split Din and the held-out split Dho. The held-in split measures whether the proposal addresses the evidence that motivated it, while the held-out split serves as a regression test for behaviors that were not visible to the proposer.

Let Pin(h) and Pho(h) denote the number of passed tasks for harness h on Din and Dho, respectively. We define the split-wise improvements of candidate h(tj) over the current harness as

∆(inj) = Pin(h(tj)) − Pin(ht), and

∆(hoj) = Pho(h(tj)) − Pho(ht).

A candidate is accepted only if it improves at least one split without degrading the other:

∆(inj) ≥ 0, ∆(hoj) ≥ 0, max ∆(inj),∆(hoj) > 0.
